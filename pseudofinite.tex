...
\def\lfin{{\mathrel{\,\raise-.6ex\rlap{\kern.3ex\sf\tiny 1}\raise.2ex\hbox{$<$}\kern.5ex }\,}}

\def\simn{{\mathrel{\raise-.5ex\rlap{\kern1.0ex\it\scriptsize n}\sim\kern.5ex }}}

\def\lsimn{{\mathrel{\,\raise-.4ex\rlap{\kern.4ex\it\tiny n}\raise.2ex\hbox{$<$}\kern.5ex }\,}}

\def\gn{{\mathrel{\,\raise-.6ex\rlap{\kern.3ex\sf\tiny n}\raise.2ex\hbox{$>$}\kern.5ex }\,}}
...
For $r,s\in {}^*\kern-.2ex\NN$ larger than $1$ we write \emph{$r\simlin s$\/} if $r\le m\, s$ and $s\le n\,r$ for some $m,n\in\NN$. We write \emph{$r\simpoly s$\/} if $r\le s^m$ and $s\le r^n$ for some $m,n\in\NN$. 
We write \emph{$r\lesslin s$\/} and \emph{$r\lesspoly s$\/} if $n\,r<s$, respectively $r^n< s$ for all $n\in\NN$.
...
\begin{corollary}
  For every $\epsilon>0$ there is an $k$ such that for every finite $M\models T_0$ of cardinality larger than $k$, there are some disjoint sets $B_0,\dots,B_{n-1}\subseteq M$, where $n\le2^m$ and $\mu(B_i)>\epsilon$, such that for every $\{{\wedge}{\vee}{\neg}\}\Delta$-definable set $A\subseteq M$ there is an $I\subseteq n$ such that 
...
\begin{lemma}
  There are two partitions of $\U$, say $\B_0,\dots,\B_n$, where $n\le 2^m$, and $\C_1,\dots,\C_{2^n}$ such that for all $i,j$ either $r(\B_i,\C_j)=_\mu\0$ or $r(\B_i,\C_j)=_\mu\B_i\times\C_j$.
\end{lemma}

\begin{proof}
  Let $\B_0,\dots,\B_{n-1}$ be the sets given by Lemma~\ref{lem_atoms} and define

  \ceq{\hfill\B_n}
  {=}
  {\U\sm\bigcup_{i=0}^{n-1}\B_i.}
  
  For every $J\subseteq n$ let 
  
  \ceq{\hfill \C_J}{=}{\Big\{c\in\U\ :\ \ \bigcup_{i\in J}\B_i\;=_\mu r(\U,c)\Big\}}

  By Lemma~\ref{lem_atoms} if $i\in J$ then $r(\B_i,\C_J)=_\mu\B_i\times\C_J$, otherwise $r(\B_i,\C_J)=_\mu\0$.

  It is only left to verify the lemma for $r(\B_n,\C_J)$.
  %
  Indeed, as $\B_n=_\mu\0$ by Lemma~\ref{lem_atoms}, we have that $r(\B_n,\C_J)\ \subseteq\ \B_n\times\U\ =_\mu\0$.
\end{proof}
...
\noindent\llap{\Large\color{red}\fontencoding{U}\fontfamily{futs}\selectfont\char 66\relax\ }%
Sembra troppo forte. Non dev'essere la traduzione al finito giusta.

\begin{corollary} 
  For every $\epsilon>0$ there is an $k$ such that for every finite $M\models T_0$ of cardinality larger than $k$, there are two partitions of $M$, say $B_0,\dots,B_n$, where $n\le 2^m$, and $C_1,\dots,C_{2^n}$, such that one of the following obtains

  \ceq{\hfill |r(B_i,C_j)|}{\le}{\epsilon\, |B_i\times C_j|}

  \ceq{\hfill |r(B_i,C_j)|}{\ge}{(1-\epsilon)\, |B_i\times C_j|.}
  \QED
\end{corollary}
