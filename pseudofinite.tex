...
\begin{corollary}
  Assume that $r(\mbox{-},\mbox{-})$ is stable and let $m=R_\Delta(\U)$.
  Then for every positive $\epsilon$ and every finite model $M$, there are two partitions of $M$, say $B_0,\dots,B_n$, where $n\le 2^m$, and $C_1,\dots,C_{2^n}$ such that 
  
  
  \ceq{\hfill r(B_i,C_j)}{\le}{\epsilon |B_i\times C_j|}

  \smallskip
  \ceq{\hfill r(B_i,C_j)}{\ge}{\epsilon |B_i\times C_j|.}
  \QED
\end{corollary}
